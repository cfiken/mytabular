...
# class TimePurchaseRunner(BaseRunner):
...
#                             base_test: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame]:
#         '''
#         train と test 両方が同時に必要な特徴を作成する。
#         '''
#         train_feature, test_feature = create_purchase_together_cd4_pca4(
#             base_train.copy(), base_test.copy(), log.copy(), category.copy(), use_cache=True)
